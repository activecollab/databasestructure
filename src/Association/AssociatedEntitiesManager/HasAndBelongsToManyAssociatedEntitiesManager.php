...
final class HasAndBelongsToManyAssociatedEntitiesManager extends BaseHasManyAssociatedEntitiesManager
{
    ...
    private $left_field_name;

    private $right_field_name;
    ...
    )
    {
        ...
        $this->left_field_name = $left_field_name;
    ...
    protected function updateAssociatedEntities(?iterable $associated_entities, int $entity_id)
    {
        if ($associated_entities !== null) {
            ...
            if (empty($reassigned_entity_ids)) {
                $finder->where("`$this->left_field_name` = ?", $entity_id);
            } else {
                $finder->where("`$this->left_field_name` = ? AND `id` NOT IN ?", $entity_id, $reassigned_entity_ids);
            }
    ...
    protected function updateAssociatedEntityIds(?array $associated_entity_ids, int $entity_id)
    {
        if ($associated_entity_ids !== null) {
            ...
            if (empty($associated_entity_ids)) {
                ...
                $finder->where("`{$this->left_field_name}` = ? OR `id` IN ?", $entity_id, $associated_entity_ids);
            }
    ...
    {
        return $entity
            ->setFieldValue($this->left_field_name, $reassign_to_entity_id)
            ->save();
    }
    ...
    ): EntityInterface
    {
        ...
        return $entity
            ->setFieldValue($this->left_field_name, null)
            ->save();
    }
}
