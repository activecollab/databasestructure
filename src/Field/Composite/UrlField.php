 ...
 */
class UrlField extends Field implements RequiredInterface, UniqueInterface, ModifierInterface, AddIndexInterface
{
    ...
    {
        $url_field = new StringField($this->getName(), $this->getDefaultValue());

        if ($this->getDefaultValue() !== null) {
            $url_field->modifier('trim');
        }
        ...
        if ($this->isUnique()) {
            $url_field->unique(...$this->getUniquenessContext());
        }
    ...
    {
        $result[] = $indent . '$validator->url(' . var_export($this->getName(), true) . ', ' . ($this->getDefaultValue() === null ? 'true' : 'false') . ');';
    }
