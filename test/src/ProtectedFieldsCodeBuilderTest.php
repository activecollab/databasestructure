...
class ProtectedFieldsCodeBuilderTest extends TestCase
{
    ...
    {
        $pool = new Pool($this->connection, $this->createMock(LoggerInterface::class));
        $logger = $this->createMock(LoggerInterface::class);

        $this->assertEquals(['field_1', 'field_2'], $this->has_protected_fields_class_reflection->newInstance($this->connection, $pool, $logger)->getProtectedFields());
        $this->assertEquals(['field_1', 'field_2', 'field_3'], $this->multi_protected_fields_class_reflection->newInstance($this->connection, $pool, $logger)->getProtectedFields());
    }
}
